...
def main():
    ...
    try:
        ...
        if x < 1000:
            xaxis.sendCommand('06',[48] + [32] + encodeToCommand(14))
            xcoordinate -= mapval(8,0,6000,0,2000)
            if xcoordinate <= 0:
                xcoordinate = 0
        elif x > 1000:
            xaxis.sendCommand('06', [49] + [32] + encodeToCommand(14))
            xcoordinate += mapval(8,0,12000,0,2000)
            if xcoordinate >= 2000:
                xcoordinate = 2000
        if y < 1000:
            yaxis.sendCommand('06', [48] + [32] + encodeToCommand(14))
            ycoordinate -= mapval(8,0,12000,0,12000)
        elif y > 1000:
            yaxis.sendCommand('06', [49] + [32] + encodeToCommand(14))
